...
def checkcode(code):
    database_file = 'C:\\Users\\hp\\Desktop\\python\\invoice\\Invoices.accdb'

    connection_string = f'DRIVER={{Microsoft Access Driver (*.mdb, *.accdb)}};DBQ={database_file};'
...
def searchinvoice(data):
    ...
    #     cursor.close()
    #     connection.close()
    #     # response={"Status":"No matching invoice","URL":None}
    #     response=None
    #     return response
    # else:
    #     query="Select LocalImageURL from Invoices where InvoiceNum=? and CustomerDop=?"
    ...
    #     cursor.execute(query,parameters)
    #     row=cursor.fetchone()
    #     # response={"Status":"Matching Invoice","URL":row[0]}
    #     response=row[0]
    #     cursor.close()
    #     connection.close()
    #     return response
